...
<!--   auc <- auc@y.values[[1]] -->
<!--   plot(ROCRperf, col="red", text.adj = c(-0.2,1.7), main="ROC Curve") -->
...
Our second ML algorithm applied to the data is Neural Networks
...
```{r}
knitr::kable(data.frame(nn_confusion_mat$overall))
```
...
```{r}
knitr::kable(data.frame(nn_confusion_mat$byClass))
```
...
We also fit K-nearest neighbors model
...
```{r}
knitr::kable(data.frame(knn_confusion_mat$overall))
```
...
```{r}
knitr::kable(data.frame(knn_confusion_mat$byClass))
```
...
We start by comparing the model outputs.

```{r, echo=T}
models_conf_mat <- list(LDA=lda_confusion_mat, NN=nn_confusion_mat, KNN=knn_confusion_mat)
all_model_results <- sapply(models_conf_mat, function(x) x$byClass)
...
Plot the correlation matrix between the models and compare their ROC.
